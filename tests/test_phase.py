...
class TestIAUHGModel:
    ...
    def test_zero_phase_angle(self):
        ...
        for G in [0.0, 0.15, 0.5, 1.0]:
            intensity = iau_hg_model(alpha, gpar=G)
            assert_allclose(intensity, 1.0, rtol=1e-10)

    @pytest.mark.parametrize("alpha_deg,G", [
        (0.0, 0.15),
        (10.0, 0.15),
        (30.0, 0.15),
        (60.0, 0.15),
    ])
    def test_specific_values(self, alpha_deg, G):
    ...
    def test_monotonic_decrease(self):
        ...
        alpha = np.array([0.0, 10.0, 30.0, 60.0, 90.0, 120.0])
        intensity = iau_hg_model(alpha, gpar=0.15)
    ...
    def test_g_parameter_effect(self):
        ...
        intensity_g0 = iau_hg_model(alpha, gpar=0.0)
        intensity_g05 = iau_hg_model(alpha, gpar=0.5)
        intensity_g1 = iau_hg_model(alpha, gpar=1.0)
    ...
    def test_array_input(self):
        ...
        alpha = np.array([0, 10, 20, 30, 60, 90])
        intensity = iau_hg_model(alpha, gpar=0.15)

        assert intensity.shape == alpha.shape
...
class TestIAUHGMag:
    ...
    def test_zero_phase_at_1au(self):
        ...
        H = 15.0
        alpha = np.array([0.0])
        mag = iau_hg_mag(H, alpha, gpar=0.15, robs=1, rhel=1)
    ...
    def test_distance_dependence(self):
        ...
        mag_1au = iau_hg_mag(H, alpha, gpar=0.15, robs=1, rhel=1)
        mag_2au_obs = iau_hg_mag(H, alpha, gpar=0.15, robs=2, rhel=1)
        mag_2au_hel = iau_hg_mag(H, alpha, gpar=0.15, robs=1, rhel=2)
    ...
    def test_phase_dependence(self):
        ...
        H = 15.0
        alpha = np.array([0.0, 30.0, 60.0, 90.0])
        mag = iau_hg_mag(H, alpha, gpar=0.15, robs=1, rhel=1)
    ...
    def test_magnitude_formula(self):
        ...
        alpha = np.array([alpha_deg])
        intensity = iau_hg_model(alpha, gpar=0.15)
        mag = iau_hg_mag(H, alpha, gpar=0.15, robs=robs, rhel=rhel)

        expected = H + 5 * np.log10(robs * rhel) - 2.5 * np.log10(intensity)
        assert_allclose(mag, expected, rtol=1e-10)

    @pytest.mark.parametrize("H,alpha_deg,robs,rhel", [
        (10.0, 0.0, 1.0, 1.0),
        (15.0, 30.0, 1.0, 1.0),
        (20.0, 0.0, 2.0, 3.0),
        (12.5, 45.0, 0.5, 1.5),
    ])
    def test_various_parameters(self, H, alpha_deg, robs, rhel):
        ...
        if alpha_deg == 0 and robs == 1 and rhel == 1:
...
class TestOppositionSurge:
    ...
    def test_surge_near_zero(self):
        ...
        alpha = np.array([0.1, 1.0, 5.0, 10.0])
        intensity = iau_hg_model(alpha, gpar=0.15)

        # Check the surge: slope should be steeper near alpha=0
        slopes = np.diff(intensity) / np.diff(alpha)
...
class TestRegressionValues:
    ...
    def test_g015_alpha_sweep(self):
        ...
        """
        alpha = np.array([0.0, 5.0, 10.0, 20.0, 40.0, 60.0, 80.0, 100.0, 120.0])
    ...
    def test_various_g_alpha30(self):
        ...
        intensities = [iau_hg_model(alpha, gpar=G)[0] for G in g_values]
